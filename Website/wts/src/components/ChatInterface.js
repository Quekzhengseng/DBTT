...
export default function ChatInterface({ conversationId, onMessageSent }) {
  ...
  const fetchMessages = async () => {
    try {
      setLoading(true)
      const response = await fetch(`http://localhost:5001/api/chat/history/${conversationId}`)
      if (response.ok) {
    ...
    return (
      ...
      >
        ...
        <p style={{ 
          ...
          Ask questions about destinations, attractions, travel packages, or get assistance with your booking.
        </p>
      </div>
    )
  }
            ...
            {loading && (
              <div style={{ display: "flex", justifyContent: "flex-start", marginTop: "8px" }}>
                <div style={{
                  ...
                  <span style={{ color: "#4b5563" }}>WTS Travel Assistant is typing...</span>
                </div>
              </div>
            )}
